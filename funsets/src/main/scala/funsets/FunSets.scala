...
object FunSets {
  ...
  def singletonSet(elem: Int): Set = {
    val singleElem = elem; (x) => x == singleElem
  }
  ...
  def exists(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = if (a > 1000) false else if (s(a)) forall(singletonSet(a), p) || iter(a + 1) else iter(a + 1)
    iter(-1000)
  }
  ...
  def map(s: Set, f: Int => Int): Set = {
    def iter(a: Int, acc: Set): Set = {
      if (a > 1000) acc
      else if (s(a)) iter(a + 1, union(singletonSet(f(a)), acc))
      else iter(a + 1, acc)
